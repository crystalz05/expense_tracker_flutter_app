...
abstract class MonthlyBudgetDao {
  @Query(
    'SELECT * FROM monthly_budgets WHERE is_deleted = 0 AND user_id = :userId ORDER BY year DESC, month DESC',
  )
  ...
  @Query(
    'SELECT * FROM monthly_budgets WHERE user_id = :userId AND month = :month AND year = :year AND is_deleted = 0',
  )
  ...
  @Query(
    'SELECT * FROM monthly_budgets WHERE user_id = :userId AND year = :year AND is_deleted = 0 ORDER BY month ASC',
  )
  ...
  @Query(
    'SELECT * FROM monthly_budgets WHERE needs_sync = 1 AND user_id = :userId',
  )
  Future<List<MonthlyBudgetModel>> getMonthlyBudgetsNeedingSync(String userId);

  @Query(
    'SELECT * FROM monthly_budgets WHERE is_deleted = 1 AND user_id = :userId',
  )
  ...
  @Query(
    'UPDATE monthly_budgets SET needs_sync = 0, last_synced_at = :syncTime WHERE id = :id',
  )
