    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `expenses` (`id` TEXT NOT NULL, `amount` REAL NOT NULL, `category` TEXT NOT NULL, `description` TEXT, `created_at` INTEGER NOT NULL, `updated_at` INTEGER NOT NULL, `payment_method` TEXT NOT NULL, PRIMARY KEY (`id`))');
...
class _$ExpenseDao extends ExpenseDao {
  ...
  Future<List<ExpenseEntity>> getExpensesByDateRange(
    ...
    return _queryAdapter.queryList(
        'SELECT * FROM expenses WHERE updated_at BETWEEN ?1 AND ?2 ORDER BY updated_at DESC',
        mapper: (Map<String, Object?> row) => ExpenseEntity(id: row['id'] as String, amount: row['amount'] as double, category: row['category'] as String, description: row['description'] as String?, createdAt: _dateTimeConverter.decode(row['created_at'] as int), updatedAt: _dateTimeConverter.decode(row['updated_at'] as int), paymentMethod: row['payment_method'] as String),
        arguments: [
  ...
  Future<List<ExpenseEntity>> getExpenseByCategory(String category) async {
    return _queryAdapter.queryList(
        'SELECT * FROM expenses WHERE category = ?1 ORDER BY updated_at DESC',
        mapper: (Map<String, Object?> row) => ExpenseEntity(
            id: row['id'] as String,
            amount: row['amount'] as double,
            category: row['category'] as String,
  ...
  Future<void> deleteExpense(String id) async {
    await _queryAdapter
        .queryNoReturn('SELECT * FROM expenses WHERE id = ?1', arguments: [id]);
  }
