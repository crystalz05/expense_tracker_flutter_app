...
abstract class ExpenseDao {
  ...
  @Query(
    'SELECT * FROM expenses WHERE is_deleted = 0 AND updated_at BETWEEN :start AND :end ORDER BY updated_at DESC',
  )
  ...
  @Query(
    'SELECT * FROM expenses WHERE is_deleted = 0 AND category = :category AND updated_at BETWEEN :start AND :end ORDER BY updated_at DESC',
  )
  ...
  @Query(
    'SELECT * FROM expenses WHERE is_deleted = 0 AND category = :category ORDER BY updated_at DESC',
  )
  Future<List<ExpenseModel>> getExpenseByCategory(String category);

  @Query(
    'SELECT SUM(amount) FROM expenses WHERE is_deleted = 0 AND category = :category',
  )
  ...
  @Query(
    'UPDATE expenses SET is_deleted = 1, updated_at = :updatedAt WHERE id = :id',
  )
